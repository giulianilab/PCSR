...
codedir = os.path.join(os.sep, "projects", group, "shared", study, "PCSR_scripts", "org", parent_dir)  # Contains subject_list.txt, config file, and dcm2bids_batch.py
configfile = os.path.join(codedir, "study_config.json")  # path to and name of config file
image = os.path.join(os.sep, "projects", group, "shared", "containers", "cbedetti-Dcm2Bids-master-latest.simg")
...
outputlog = os.path.join(logdir, "outputlog_dcm2bids" + datetime.now().strftime("%Y%m%d-%H%M") + ".txt")
